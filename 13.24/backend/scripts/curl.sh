...
$ curl -X "POST" http://localhost:3001/api/login -H "Content-Type: application/json" -d "{\"username\":\"fullstack@fullstack.com\", \"password\":\"fullstack\"}"

$ curl -X "DELETE" http://localhost:3001/api/logout -H "Content-Type: application/json" -d "{\"username\":\"fullstack@fullstack.com\"}"

$ curl -X "GET" http://localhost:3001/api/users/1

$ curl -X "GET" http://localhost:3001/api/users/1?read=true

$ curl -X "GET" http://localhost:3001/api/users/1?read=false
...
$ curl -X "PUT" http://localhost:3001/api/readinglists/1 -H "Content-Type: application/json" -H "Authorization: bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VybmFtZSI6ImZ1bGxzdGFja0BmdWxsc3RhY2suY29tIiwiaWF0IjoxNjQwNTAyMjk3fQ.C8zPuXwUa5ZByCNC-rlCSLUJcUaszzftSmM5q7yMhOk" -d '{"read":"true"}'
